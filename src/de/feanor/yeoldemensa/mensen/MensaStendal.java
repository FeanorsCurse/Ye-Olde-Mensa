 ...
 * 
 */
public class MensaStendal extends Mensa {
	...
	protected void loadMenu() throws IOException {
		...
		SimpleHTMLTokenizer tokenizer = new SimpleHTMLTokenizer(
				new URL(
						"http://www.studentenwerk-magdeburg.de/deutsch/essen_trinken/speiseplan/seiten/mensa_stendal.aspx"),
				"utf-8");
		...
		while ((element = tokenizer.nextText()) != null) {
			if (((element.startsWith("Essen 1"))
					|| (element.startsWith("Essen 2"))
					|| (element.startsWith("Essen 3")) || (element
					.startsWith("Essen 4")))
				...
				this.addMenuItem(new MenuItem(day, getMenuType(menuType), output));
			}
			if (element.startsWith("Beilagen:") && element.length() > 9) {
				...
				for (int i = 0; i < elements.length; i++) {
					this.addMenuItem(new MenuItem(day, getMenuType(menuType), elements[i]));
				}
			}
		}
	}
