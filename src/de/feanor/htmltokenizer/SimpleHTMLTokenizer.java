...
public class SimpleHTMLTokenizer {
	...
	public SimpleHTMLTokenizer(String urlString, String encoding, String date)
			throws IOException {
		...
		String dataset = URLEncoder
				.encode(
						"ctl00%24ctl00%24_PlaceHolderLeftNavBarTop%24SearchBox%24S5F437507_InputKeywords",
						"UTF-8")
				+ "=";
		dataset += "&"
				+ URLEncoder.encode("__EVENTTARGET", "UTF-8")
				+ "="
				+ URLEncoder
						.encode(
								"ctl00$ctl00$_PlaceHolderCalendarNavigator$PlaceHolderCalendarNavigator$MensaKalender",
								"UTF-8");
		dataset += "&" + URLEncoder.encode("__EVENTARGUMENT", "UTF-8") + "="
		...
		while ((line = br.readLine()) != null) {
			if (line.matches("^<input.*")) {
				...
				dataset += "&" + URLEncoder.encode(name, "UTF-8") + "="
