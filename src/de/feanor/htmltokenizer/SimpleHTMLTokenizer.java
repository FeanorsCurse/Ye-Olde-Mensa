...
/**
 * Quick'n'dirty implementation of a HTML tokenizer (/parser). See parse(BufferedReader) comment for hack :P.
 * 
 * @author Daniel Süpke
 *
 */
public class SimpleHTMLTokenizer {
	...
	// TODO: This parser is quite bad. After each tag it always assumes a text, which will be ignored if a tag.
	// Also, the char-copy operations could probably be much faster.
	// However, I found no gpl+up-to-date parser out there, so this is a quick hack.
	private void parse(BufferedReader br) throws IOException {
		...
		do {
			...
			if (c == '<') {
				...
				tokenList.add(new Token(new String(buf, 0, size).toLowerCase(), TAG));
				
				// TODO: Worst finish condition evar
				finished = new String(buf, 0, size).toLowerCase().equals("/html");
			}
	...
	/**
	 * Pushes back the last element, so it can be read again with nextTag ornextElement
	 */
