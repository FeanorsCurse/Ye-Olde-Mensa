...
public class SimpleGSMHelper {
	...
	private double[] getCoordinatesFromGSM(int cellID, int lac, int mnc, int mcc) {
		...
		try {
			/*
			 * Opening Connection to OpenCellID geo localisation database and
			 * parsing the txt return values
			 */
			...
			String urlString = "http://www.opencellid.org/cell/get?cellid="
					+ cellID + "&mcc=" + mcc + "&mnc=" + mnc + "&lac=" + lac
					+ "&fmt=txt";
			...
			/*
			 * Reading input stream byte by byte and appending result to the
			 * string buffer
			 */
			...
			if (res.startsWith("err")) {
				/*
				 * In case of an error set default longitude and latitude valuse
				 * to 0.0, 0.0
				 */
				result[0] = 0.0;
				result[1] = 0.0;
	...
	public double getDistance(double[] targetCoordinates) {
		...
		/*
		 * This method retrieves the distance of the mobile phone to a target
		 * location that is stored in the input double array The array consists
		 * of a latitude value at position 0 and a longitude value at position 1
		 */

		double[] sourceCoordinates = this.getCoordinatesFromGSM(this.cellID,
				this.lac, this.mnc, this.mcc);
		...
		/*
		 * Trigonometrical function to calculate the real distance based on the
		 * earth norm elipsoid
		 */

		double dist = Math.abs(Math.acos(Math.sin(lat1) * Math.sin(lat2)
				+ Math.cos(lat1) * Math.cos(lat2) * Math.cos(lng1 - lng2))) * 6370.137;
	...
	public void setMobileLocation(TelephonyManager manager) {
		...
		try {
			...
			/*
			 * This method sets the current location of the mobile phone based
			 * on its operative mobile state
			 */
			...
			if (this.iso.equalsIgnoreCase("de")
					&& (tm.getNetworkOperator().length() == 5)) {

				/*
				 * For germany the iso code must be "de" and the string
				 * comprising mobile country code (mcc) and mobile network code
				 * (mnc) must have a length of 5 digits (3 mcc and 2 mnc)
				 * furthermore the cellID and the lac is needed for the Open
				 * CellID service
				 */
				...
				Integer mcc = Integer.parseInt(tm.getNetworkOperator()
						.substring(0, 3));
				this.mcc = mcc.intValue();
				Integer mnc = Integer.parseInt(tm.getNetworkOperator()
						.substring(3, 5));
